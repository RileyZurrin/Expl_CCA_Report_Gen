...
#Exploratory CCA Files. Enter data for all groups.
canoload = ['canoload_RSPM_con.xlsx', 'canoload_RSPM_pat.xlsx']
CCA = ['CCA_RSPM_con.csv','CCA_RSPM_pat.csv']
PCA = ['PCA_RSPM_con.csv','PCA_RSPM_pat.csv']
...
if num_groups == 1:
    mydoc.add_paragraph(
        "This report is for the ________ (" + Task + ") task, in which participants were _____. There were " + str(
            len(conditions))
        + ' conditions: ' + constring + ' Subjects in the _____ condition did ___(describe condition 1)___, subjects in the ____ condition '
                                        'did __(describe condition 2)___, etc. The ' + str(
...
if num_groups > 1:
    mydoc.add_paragraph(
        "This report is for the ________ (" + Task + ") task, in which participants were _____. There were " + str(
            num_groups) + " groups: controls and patients with ___(e.g. bipolar disorder)___. There were " + str(
            len(conditions))
        + ' conditions: ' + constring + ' Subjects in the _____ condition did ___(describe condition 1)___, subjects in the ____ condition '
                                        'did __(describe condition 2)___, etc. The ' + str(
            len(network)) + ' networks identified as part of this study are: '
...
for p in range(num_groups):
    if num_groups == 1:
        ...
        mydoc.add_heading("Summary of Results for group " + str(p), 1)
        mydoc.add_heading("Summary for Each Behavioural Variable", 2)
