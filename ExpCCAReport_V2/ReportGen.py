...
canoload = ['canoload_LDT_Con.xlsx', 'canoload_LDT_Bip.xlsx']    # e.g. if num_groups > 1: ['canoload_RSPM_con.xlsx', 'canoload_RSPM_pat.xlsx']
...
if len(Groups) > 1:
    mydoc.add_paragraph(
        "This report is for the ________ (" + Task + ") task, in which participants were ___(describe task)___. There were " + str(
            len(Groups)) + " groups: controls and patients with ___(e.g. bipolar disorder)___. There were " + str(
            len(conditions))
        + ' conditions: ' + constring + ' Subjects in the _____ condition did ___(describe condition 1)___, subjects in the ____ condition '
                                        'did __(describe condition 2)___, etc. The ' + str(
            len(network)) + ' networks identified as part of this study are: '
